...
export function bearer(){
  const t = (typeof window !== 'undefined' && localStorage.getItem('sb_access_token')) || ''
...
export async function generateDraft(payload){
  const r = await fetch(`${BASE}/api/draft`, { method:'POST', headers:{ 'Content-Type':'application/json', ...bearer() }, body: JSON.stringify(payload||{}) })
  return toJson(r)
}

export async function tool(path, payload){
  const r = await fetch(`${BASE}${path}`, { method:'POST', headers:{ 'Content-Type':'application/json', ...bearer() }, body: JSON.stringify(payload||{}) })
  return toJson(r)
}
