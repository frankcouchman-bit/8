  ...
  return (
    <ToolCard title="Research (debug)" description="Run the Worker research utility.">
      <div className="grid md:grid-cols-3 gap-4">
        <label className="block"><div className="text-sm text-white/70 mb-1">Query</div><input value={q} onChange={e=>setQ(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10"/></label>
        ...
        <div className="md:col-span-3">
          <button disabled={loading || blocked} onClick={async ()=>{ setLoading(true); setResult(null); const r=await researchDebug(q, region); setResult(r); setLoading(false) }} className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 border border-white/10">Run Research</button>
        </div>
      </div>
