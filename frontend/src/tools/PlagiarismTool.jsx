...
const [text, setText] = React.useState('Paste your text...')
  ...
  return (
    <ToolCard title="Plagiarism" description="Originality & similarity check.">
      ...
      <div className="mt-3">
        <button disabled={loading || blocked} onClick={async ()=>{ setLoading(true); setResult(null); const r=await ToolsAPI.plagiarism({ text }); setResult(r); setLoading(false) }} className="px-4 py-2 rounded-xl bg-brand-500 hover:bg-brand-400 shadow-premium">{loading?'Running…':'Check Originality'}</button>
      </div>
