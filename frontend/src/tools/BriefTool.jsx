...
const [keyword, setKeyword] = React.useState('article writer')
  ...
  return (
    <ToolCard title="Brief Builder" description="Build a SERP-aware brief.">
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-3">
          <button disabled={loading || blocked} onClick={async ()=>{ setLoading(true); setResult(null); const r=await ToolsAPI.brief({ keyword, region }); setResult(r); setLoading(false) }} className="px-4 py-2 rounded-xl bg-brand-500 hover:bg-brand-400 shadow-premium">{loading?'Running…':'Build Brief'}</button>
        </div>
      </div>
