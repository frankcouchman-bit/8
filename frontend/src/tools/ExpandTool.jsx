...
export default function ExpandTool(){
...
const [context, setContext] = React.useState('Existing content...')
  const [region, setRegion] = React.useState('us')
  const [keyword, setKeyword] = React.useState('article writer')
  const [loading, setLoading] = React.useState(false)
  const [result, setResult] = React.useState(null)
  return (
    <ToolCard title="Expand Article" description="Expand an existing draft.">
      <label className="block"><div className="text-sm text-white/70 mb-1">Current article (markdown or text)</div><textarea value={context} onChange={e=>setContext(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 min-h-[140px]" /></label>
      ...
      <div className="mt-3">
        <button disabled={loading || blocked} onClick={async ()=>{ setLoading(true); setResult(null); const r=await ToolsAPI.expand({ context, region, keyword }); setResult(r); setLoading(false) }} className="px-4 py-2 rounded-xl bg-brand-500 hover:bg-brand-400 shadow-premium">{loading?'Running…':'Expand'}</button>
      </div>
