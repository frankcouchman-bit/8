...
export default function KeywordsTool(){
...
const [topic, setTopic] = React.useState('article writer')
  const [region, setRegion] = React.useState('us')
  const [loading, setLoading] = React.useState(false)
  const [result, setResult] = React.useState(null)
  return (
    <ToolCard title="Keyword Cluster" description="Cluster keywords by intent and topic.">
      <div className="grid md:grid-cols-3 gap-4">
        <label className="block"><div className="text-sm text-white/70 mb-1">Seed</div><input value={topic} onChange={e=>setTopic(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10"/></label>
        ...
        <div className="md:col-span-3">
          <button disabled={loading || blocked} onClick={async ()=>{ setLoading(true); setResult(null); const r=await ToolsAPI.keywords({ topic, region }); setResult(r); setLoading(false) }} className="px-4 py-2 rounded-xl bg-brand-500 hover:bg-brand-400 shadow-premium">{loading?'Running…':'Cluster Keywords'}</button>
        </div>
      </div>
